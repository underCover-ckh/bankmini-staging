...
<div class="p-6 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-100 rounded-lg shadow-lg transition-colors duration-200">
    <h2 class="text-2xl font-bold text-blue-600 dark:text-blue-400 mb-6">Edit User</h2>
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="Nama Lengkap" required 
        ...
        <div>
            ...
            <input type="text" id="nis" name="nis" value="{{ old('nis', $user->nis) }}" placeholder="Nomor Induk Siswa" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select id="kelas" name="kelas"
                    ...
                    <option value="x" {{ old('kelas', strtolower($user->kelas)) == 'x' ? 'selected' : '' }}>X</option>
                    <option value="xi" {{ old('kelas', strtolower($user->kelas)) == 'xi' ? 'selected' : '' }}>XI</option>
                    <option value="xii" {{ old('kelas', strtolower($user->kelas)) == 'xii' ? 'selected' : '' }}>XII</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select id="jurusan" name="jurusan"
                    ...
                    <option value="tjkt1" {{ old('jurusan', strtolower($user->jurusan)) == 'tjkt1' ? 'selected' : '' }}>TJKT 1</option>
                    <option value="tjkt2" {{ old('jurusan', strtolower($user->jurusan)) == 'tjkt2' ? 'selected' : '' }}>TJKT 2</option>
                    <option value="pplg" {{ old('jurusan', strtolower($user->jurusan)) == 'pplg' ? 'selected' : '' }}>PPLG</option>
                    <option value="akl" {{ old('jurusan', strtolower($user->jurusan)) == 'akl' ? 'selected' : '' }}>AKL</option>
                    <option value="acp" {{ old('jurusan', strtolower($user->jurusan)) == 'acp' ? 'selected' : '' }}>ACP</option>
                </select>
            </div>
        </div>
